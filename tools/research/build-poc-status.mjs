...
const lane = (platform, runtime, ready, manifest) => ready
  ? { runtime, status: "built", harnessPath: platform === "ios" ? "pocs/ios-coreai/main.swift" : "pocs/android-executorch/Main.kt", reason: "Parameterized harness is admitted; physical benchmark must still emit a validated result." }
  : { runtime, status: "blocked", harnessPath: platform === "ios" ? "pocs/ios-coreai/main.swift" : "pocs/android-executorch/Main.kt", reason: platform === "ios" ? `Core AI POC blocked: requires Xcode/iOS 27 and an available physical iPhone; observed ${manifest?.toolchain?.version ?? "no manifest"}.` : `ExecuTorch POC blocked: requires Android SDK/adb and an available physical Android device; observed ${(manifest?.reason ?? "no manifest").replace(/\.$/, "")}.` };
const records = inventory.verifiedIntersection.map((modelId) => {
  const record = inventory.records.find((entry) => entry.modelId === modelId);
  const result = { pocId: `poc-${modelId.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase()}`, modelId, modelRevision: record.revision, fixtureId: "text-generation/basic-v1", ios: lane("ios", "core-ai", iosReady, ios), android: lane("android", "executorch", androidReady, android) };
